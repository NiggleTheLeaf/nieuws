...
After the initial setup, run `SECURITY-HARDENING.sql` once in the Supabase SQL Editor. It reapplies least-privilege grants, hardens the staff-check function, forces RLS on all exposed tables, and bounds public analytics input. The migration is designed to be safely rerun.

The Project URL and `sb_publishable_...` key are intentionally public identifiers. Never put an `sb_secret_...` or legacy `service_role` key in the repository or any browser file.
